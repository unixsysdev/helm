...
def prepare_mixed_data(tokenizer, seq_len, target_tokens=2_000_000_000, hf_token=None):
    """Stream and tokenize from FineWeb-Edu (1.3T educational web tokens).

    High-quality educational content including technical, scientific, and code material.
    Note: Nebius SWE-agent trajectories reserved for KL distillation phase.
    """
    cache_path = Path('data') / f'fineweb_edu_{seq_len}_v1'
    ...
    print(f"Streaming FineWeb-Edu (target: {target_tokens:,} tokens)...")
    ...
    for example in ds:
        ...
        if len(text) > 200:
            ...
            doc_count += 1
        if doc_count % 50000 == 0 and doc_count > 0:
            print(f"  FineWeb-Edu: {doc_count:,} docs, {total_tokens:,} tokens")
    print(f"  FineWeb-Edu done: {doc_count:,} docs, {total_tokens:,} tokens")

    total = len(all_ids)
    print(f"\n{'='*60}")
    print(f"Dataset: {total:,} tokens from {doc_count:,} documents")
    print(f"{'='*60}")
...
def main():
    ...
    parser.add_argument('--rollback_save_every', type=int, default=200)
    ...
    # Data
    dataset = prepare_mixed_data(tokenizer, args.seq_len, args.target_tokens, args.hf_token)
